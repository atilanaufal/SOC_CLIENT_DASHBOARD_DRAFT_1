 ...
 * is synchronized with Better Auth `users` table.
 */
export async function syncMasterUserToBetterAuth(
  ...
  try {
    ...
    const userEmail = masterUser.email || masterUser.username;

    // 2. Check if user already exists in `users` table
    const [existingRows]: any = await authDbPool.query(
      'SELECT id, email, name, role, tenant_id FROM users WHERE name = ? OR email = ? LIMIT 1',
    ...
    if (existingRows && existingRows.length > 0) {
      ...
      // Update tenant metadata in users table (only fields that exist in the users schema)
    ...
    // 3. User does not exist in table yet -> Register into Better Auth
    try {
