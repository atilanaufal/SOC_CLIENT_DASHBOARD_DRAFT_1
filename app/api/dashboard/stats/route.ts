...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      const vulnRes = await getTenantVulnerabilities(tenant.databaseName, tenant.redisPrefix, timeFilter, startDate, endDate, 0, 1000);
      const vulns = Array.isArray(vulnRes) ? vulnRes : (vulnRes.data || []);
      ...
      vulnTotal = (vulnRes as any).total ?? filteredVulns.length;
