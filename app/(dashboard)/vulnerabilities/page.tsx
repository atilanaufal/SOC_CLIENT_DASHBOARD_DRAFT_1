...
function VulnerabilitiesContent() {
  ...
  // Refs for tracking background polling interval and active load cycle
  const intervalRef = React.useRef<NodeJS.Timeout | null>(null);
  const cycleIdRef = React.useRef<number>(0);

  // Real-time Database fetch with progressive 1,000 batch loading and session caching
  const loadData = async (forceRefresh = false) => {
    // Increment cycle ID to invalidate any background tasks from previous cycles
    ...
    const cacheKey = `vulnerabilities:${timeFilter}:${customRange?.startDate || ''}:${customRange?.endDate || ''}`;

    // If not forcing refresh, check if data is already cached for this session
    if (!forceRefresh) {
      try {
        const cached = await getClientCache<{ data: Vulnerability[]; total: number; isComplete: boolean }>(cacheKey);
        if (cycleIdRef.current !== currentCycle) return;

        if (cached && Array.isArray(cached.data) && cached.data.length > 0) {
          setVulnerabilities(cached.data);
          setTotalCount(cached.total || cached.data.length);
          setLoading(false);

          // If previously 100% completed, we are done! Zero network calls, 0ms render!
          if (cached.isComplete) {
            setIsLoadingMore(false);
            return;
          }

          // If only partially loaded previously, resume background chunks from cached count
          let currentOffset = cached.data.length;
          const total = cached.total || cached.data.length;
          ...
          intervalRef.current = setInterval(async () => {
            ...
            try {
              const nextBatch = await fetchVulnerabilities({
              ...
              const nextData = nextBatch.data || [];
              if (nextData.length === 0) {
                if (intervalRef.current) clearInterval(intervalRef.current);
                setIsLoadingMore(false);
                setClientCache(cacheKey, { data: cached.data, total, isComplete: true });
                return;
              }

              setVulnerabilities((prev) => {
                const existingKeys = new Set(
                  prev.map((v) => v.id || `${v.cveId}_${v.name}_${v.agent}_${v.detectionDate}`)
                );
                const uniqueNew = nextData.filter(
                  (v) => !existingKeys.has(v.id || `${v.cveId}_${v.name}_${v.agent}_${v.detectionDate}`)
                );
                const updated = [...prev, ...uniqueNew];
                const isDone = updated.length >= total || !nextBatch.hasMore;
                setClientCache(cacheKey, { data: updated, total, isComplete: isDone });
                return updated;
              });

              currentOffset += 1000;
              if (currentOffset >= total || !nextBatch.hasMore) {
                if (intervalRef.current) clearInterval(intervalRef.current);
                setIsLoadingMore(false);
              }
            } catch (chunkErr) {
              console.warn('[Vulnerabilities] Background chunk fetch warning:', chunkErr);
              if (intervalRef.current) clearInterval(intervalRef.current);
              setIsLoadingMore(false);
            }
          }, 2000);
    ...
    try {
      ...
      // 1. Initial immediate batch: 1,000 items
      const firstBatch = await fetchVulnerabilities({
        timeRange: timeFilter,
        startDate: customRange?.startDate,
        endDate: customRange?.endDate,
        offset: 0,
      ...
      const initialData = firstBatch.data || [];
      const total = firstBatch.total || initialData.length;
      const isCompleteInitially = !firstBatch.hasMore || initialData.length >= total;

      setVulnerabilities(initialData);
      setTotalCount(total);
      setLoading(false); // First batch renders immediately in ~1-1.5s!

      // Cache initial batch
      setClientCache(cacheKey, {
        data: initialData,
        total,
        isComplete: isCompleteInitially,
      });

      // 2. If there is more data, query 1,000 items every 2 seconds
      if (firstBatch.hasMore && initialData.length < total) {
        setIsLoadingMore(true);
        ...
        intervalRef.current = setInterval(async () => {
          ...
          try {
            ...
            if (nextData.length === 0) {
              if (intervalRef.current) clearInterval(intervalRef.current);
              setIsLoadingMore(false);
              setClientCache(cacheKey, { data: initialData, total, isComplete: true });
              return;
            }
          ...
          <div className="md:col-span-5 bg-white/70 backdrop-blur-xl p-3.5 sm:p-4 xl:p-5 rounded-xl border border-white/70 shadow-[0_8px_32px_0_rgba(31,38,135,0.04),inset_0_1px_1px_0_rgba(255,255,255,0.9)] flex items-center justify-around gap-3">
            ...
            <div className="space-y-1 text-xs sm:text-xs xl:text-sm font-bold">
              ...
              <div className="flex items-center justify-between gap-3 sm:gap-4 text-red-700 font-bold">
                ...
                <span className="text-gray-900 text-sm sm:text-base font-black">{stats.critical}</span>
              </div>
              <div className="flex items-center justify-between gap-3 sm:gap-4 text-orange-600 font-bold">
                ...
                <span className="text-gray-900 text-sm sm:text-base font-black">{stats.high}</span>
              </div>
              <div className="flex items-center justify-between gap-3 sm:gap-4 text-[#0066B1] font-bold">
                ...
                <span className="text-gray-900 text-sm sm:text-base font-black">{stats.medium}</span>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 bg-white/70 backdrop-blur-xl p-3.5 sm:p-4 xl:p-5 rounded-xl border border-white/70 shadow-[0_8px_32px_0_rgba(31,38,135,0.04),inset_0_1px_1px_0_rgba(255,255,255,0.9)] flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2 mt-0.5">
                <span className="text-3xl sm:text-3xl xl:text-4xl font-black text-emerald-700">{stats.solved}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-xl border border-white/70 shadow-[0_8px_32px_0_rgba(31,38,135,0.04),inset_0_1px_1px_0_rgba(255,255,255,0.9)] flex-1 flex flex-col justify-between min-w-0 overflow-hidden">
          ...
          <div className="bg-white/80 backdrop-blur-md border-t border-white/60 px-3.5 sm:px-4 py-2.5 sm:py-3 flex flex-col sm:flex-row gap-2.5 sm:gap-3 items-center justify-between text-xs sm:text-sm font-semibold text-gray-800 flex-shrink-0 shadow-[inset_0_1px_1px_rgba(255,255,255,0.8)]">
            <div className="flex flex-wrap items-center gap-2">
              <span>
                Showing {filteredVulns.length === 0 ? 0 : startIndex + 1}-{Math.min(startIndex + pageSize, filteredVulns.length)} of {filteredVulns.length.toLocaleString()} Vulnerabilities
              </span>
            </div>
            <Pagination
