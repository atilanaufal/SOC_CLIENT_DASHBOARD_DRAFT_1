...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadStats(forceRefresh = false) {
      const cacheKey = `dashboard_stats:${timeFilter}:${customRange?.startDate || ''}:${customRange?.endDate || ''}`;

      if (!forceRefresh) {
        try {
          const cached = await getClientCache<any>(cacheKey);
          if (cached) {
            setStatsData(cached);
            setIsLoading(false);
            return;
  ...
  return (
    <div className="w-full flex flex-col gap-3.5 sm:gap-4 md:gap-4.5 xl:gap-5 2xl:gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3.5 sm:gap-4 md:gap-4.5 xl:gap-5 2xl:gap-6 items-stretch w-full">
        ...
        {/* 3. Top Incident */}
        <div className="md:col-span-2 lg:col-span-5 xl:col-span-5 2xl:col-span-5 bg-white/80 backdrop-blur-xl rounded-xl border border-white/80 flex flex-col justify-between relative z-20 overflow-hidden h-full max-h-[460px] xl:max-h-[480px] shadow-[0_8px_32px_0_rgba(31,38,135,0.04),inset_0_1px_1px_0_rgba(255,255,255,0.9)]">
          ...
          {/* Item Container - Scrollable with custom scrollbar, prevents card stretching */}
          <div className="divide-y divide-gray-100/80 flex-1 flex flex-col justify-start px-1 overflow-y-auto rounded-b-xl relative z-10">
            {isLoading ? (
